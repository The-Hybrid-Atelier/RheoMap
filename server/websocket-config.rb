...
$options = {b: "0.0.0.0", p: 3040, v: false}

$client = Mongo::Client.new([ 'XXX.XX.XXX.X:XXXXX' ], 
   user: 'XXXX',
   password: 'XXXX',
   database: 'XXX',
   auth_source: 'XXX',
...
class Routes
  ...
  def multicast(prefix="", ws, msg)
  	@multicast = @websockets.reject { |sid, ms| [ws.signature].include? sid }
  	if @multicast.empty? then return end
    @multicast.each{ |sid, ms| ms.jsend(ws.name, msg)}
  ...
  def unicast(prefix="", ws, msg)
    ...
    @unicast.each do |sid, ms| 
      ms.jsend(ws.name, msg)
    end
    puts "\tUC: #{prefix} sent to #{@unicast.map{ |sid, ms| ms.name}} devices.. from #{ws.name}\n"
  end
...
def start_server
  EventMachine.run do
    ...
    $routes = Routes.new()
    EM::WebSocket.start(:host => $options[:b], :port => $options[:p], :debug => $options[:d]) do |ws|
